...
internal class FTPGateway
{
	...
	internal static void MakeFTPRequest(Session oSession, PipeReadBuffer buffBody, out HTTPResponseHeaders oRH)
	{
		...
		else
		{
			...
			{
				oFTP.UseBinary = FiddlerApplication.Prefs.GetBoolPref("fiddler.ftp.UseBinary", bDefault: true);
			}
		}
		...
		else if (oSession.oRequest.headers.ExistsAndContains("Authorization", "Basic "))
		{
			...
			string sPass3 = Utilities.TrimBefore(sAuth2, ':');
			oFTP.Credentials = new NetworkCredential(sUser2, sPass3);
		}
		else if (oSession.oFlags.ContainsKey("x-AutoAuth") && oSession.oFlags["x-AutoAuth"].Contains(":"))
		{
			...
			string sPass2 = Utilities.TrimBefore(oSession.oFlags["x-AutoAuth"], ':');
			oFTP.Credentials = new NetworkCredential(sUser3, sPass2);
		}
		else if (FiddlerApplication.Prefs.GetBoolPref("fiddler.ftp.AlwaysDemandCredentials", bDefault: false))
		{
			byte[] arrErrorBody2 = Encoding.UTF8.GetBytes("Please provide login credentials for this FTP server".PadRight(512, ' '));
		...
		oFTP.UsePassive = FiddlerApplication.Prefs.GetBoolPref("fiddler.ftp.UsePassive", bDefault: true);
		oFTP.Proxy = null;
		...
		catch (WebException eX)
		{
			...
			if (ftwError != null)
			{
				...
				{
					arrErrorBody = Encoding.UTF8.GetBytes("This FTP server requires login credentials".PadRight(512, ' '));
				...
				arrErrorBody = Encoding.UTF8.GetBytes(string.Format("{0}{1}{2}", "Fiddler was unable to act as a HTTP-to-FTP gateway for this response. ", ftwError.StatusDescription, string.Empty.PadRight(512, ' ')));
				buffBody.Write(arrErrorBody, 0, arrErrorBody.Length);
			}
			else
			{
				byte[] arrErrorBody = Encoding.UTF8.GetBytes(string.Format("{0}{1}{2}", "Fiddler was unable to act as a HTTP-to-FTP gateway for this response. ", eX.Message, string.Empty.PadRight(512, ' ')));
				buffBody.Write(arrErrorBody, 0, arrErrorBody.Length);
			}
			oRH = new HTTPResponseHeaders();
