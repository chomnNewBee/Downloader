...
internal class SimpleTracker
{
	...
	private IDictionary<string, string> AddRequiredHitData(IDictionary<string, string> @params)
	{
		...
		{
			result.Add("sr", $"{ScreenResolution.Value.Width}x{ScreenResolution.Value.Height}");
		}
		if (ViewportSize.HasValue)
		{
			result.Add("vp", $"{ViewportSize.Value.Width}x{ViewportSize.Value.Height}");
		}
		if (Language != null)
		{
			result.Add("ul", Language);
		}
		if (ScreenColors.HasValue)
		{
			result.Add("sd", $"{ScreenColors.Value}-bits");
		}
		if (Referrer != null)
		{
		...
		{
			result.Add("geoid", LocationOverride);
		}
		foreach (KeyValuePair<string, string> item in data.Concat(@params))
		{
	...
	{
		if (!string.IsNullOrEmpty(PropertyId) && !IsSampledOut())
		{
	...
	private bool IsSampledOut()
	{
		...
		if (SampleRate < 100f)
		{
			return ClientId != null && (float)(Math.Abs(ClientId.GetHashCode()) % 10000) >= SampleRate * 100f;
		}
		return false;
	}
}
