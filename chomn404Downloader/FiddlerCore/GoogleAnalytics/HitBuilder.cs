...
/// </summary>
internal sealed class HitBuilder
{
	private const string HitType_Screenview = "screenview";
	...
	private readonly IList<HitBuilder> lineage;

	private readonly IDictionary<string, string> data;

	private IDictionary<string, string> Data => data;
	...
	public static HitBuilder CreateCustomEvent(string category, string action, string label = null, long value = 0L)
	{
		...
		if (value != 0)
		{
			data.Add("ev", value.ToString(CultureInfo.InvariantCulture));
		}
	...
	{
		Dictionary<string, string> data = new Dictionary<string, string>();
		data.Add("t", "screenview");
		if (screenName != null)
		{
			data.Add("cd", screenName);
		}
		return new HitBuilder(data);
	}
	...
	public static HitBuilder CreateTiming(string category, string variable, TimeSpan? value, string label = null)
	{
		...
		if (category != null)
		{
			data.Add("utc", category);
		}
		if (variable != null)
		{
			data.Add("utv", variable);
		}
		if (value.HasValue)
		{
	...
	{
		return new HitBuilder(lineage, new Dictionary<string, string> { 
		{
			$"cd{index}",
			dimension
		} });
	...
	public HitBuilder AddProduct(Product product)
	{
		...
		foreach (KeyValuePair<int, string> item2 in product.CustomDimensions)
		{
			data.Add($"pr{index}cd{item2.Key}", item2.Value);
		}
		foreach (KeyValuePair<int, int> item in product.CustomMetrics)
		{
	...
	public HitBuilder AddPromotion(Promotion promotion)
	{
		...
		{
			data.Add($"promo{index}id", promotion.Id);
		}
		if (promotion.Name != null)
		{
			data.Add($"promo{index}nm", promotion.Name);
		}
		if (promotion.Creative != null)
		{
			data.Add($"promo{index}cr", promotion.Creative);
		}
		if (promotion.Position != null)
		{
			data.Add($"promo{index}ps", promotion.Position);
		}
		return new HitBuilder(lineage, data)
		{
			PromotionCount = index
		};
	}
	...
	public HitBuilder SetProductAction(ProductAction action)
	{
		...
		{
			data.Add("pa", action.Action);
		}
		if (action.TransactionId != null)
		{
			data.Add("ti", action.TransactionId);
		}
		if (action.TransactionAffiliation != null)
		{
			data.Add("ta", action.TransactionAffiliation);
		}
		...
		if (action.TransactionCouponCode != null)
		{
			data.Add("tcc", action.TransactionCouponCode);
		}
		if (action.ProductActionList != null)
		{
			data.Add("pal", action.ProductActionList);
		}
		if (action.CheckoutStep.HasValue)
		{
			data.Add("cos", action.CheckoutStep.Value.ToString(CultureInfo.InvariantCulture));
		}
		if (action.CheckoutOptions != null)
		{
			data.Add("col", action.CheckoutOptions);
		}
		return new HitBuilder(lineage, data);
	}
	...
	public IDictionary<string, string> Build()
	{
		...
		foreach (HitBuilder hitBuilder in lineage)
		{
			foreach (KeyValuePair<string, string> item in hitBuilder.Data)
			{
